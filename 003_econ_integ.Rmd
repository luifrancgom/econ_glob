...
european_union <- codelist_panel %>%
    filter(!is.na(eu28) & year == 2020) %>% 
...
ne_countries(scale = 10, returnclass = "sf") %>%
    ...
    mutate(n_rta = as.character(n_rta),
           n_rta = case_when(
               iso_a3 == "COL" ~ "Colombia",
               TRUE ~ n_rta),
           n_rta = fct_relevel(n_rta, c("0", "1", "2", "3", "4", "5", "Colombia"))) %>%
  
    # Plot
    ggplot() +
    geom_sf(aes(geometry = geometry, fill = n_rta), color = "black", alpha = 0.5) + 
    scale_fill_manual(values = c("white", palette_light()[1:6])) +
...
read_csv("003_all_rta.csv") %>% 
    ...
    footnote(number = c("Source: RTA database - WTO & OMC", "Last update: 2020-07-31"))
```
...
ne_countries(returnclass = "sf", type = "countries", scale = 10) %>%
    ...
    mutate(trade_region = case_when(
                            ...
                            adm0_a3 %in% c("ATG", "BHS", "BRB",
                                           ...
                                           "MSR", "KNA", "LCA", 
                                           "VCT", "SUR", "TTO") ~ "CARICOM",
                            TRUE ~ "Not in CACM or CARICOM"),
        trade_region = fct_relevel(trade_region, c("Not in CACM or CARICOM", "CACM", "CARICOM"))) %>% 
    ...
    labs(fill     = "",
         title    = str_glue("Central American Common Market (CACM) and 
