...
- Knowing the data
    ...
    + __Data source__: https://www.banrep.gov.co/ > Estadísticas > Tasas de cambio, sector externo y derivados > 1. Tasas de cambio > Tasa Representativa del Mercado (TRM - Peso por dólar) > Descargar y consultar: Serie histórica para un rango de fechas dado (desde 27/11/1991)
...
cop_usd_trm %>% 
    ggplot() + 
    geom_histogram(aes(x = trm, y = ..density..),
                   bins     = bins + 1, 
                   color    = "black", 
                   fill     = palette_light()[[1]],
                   alpha    = 0.8,
...
i_col_usa_trm_forecast %>% 
    ggplot() + 
    geom_histogram(aes(x = error, y = ..density..),
                   bins = bins + 1, 
                   color = "black", 
                   fill  = palette_light()[[1]],
                   alpha = 0.8, 
