...
# Data
col_2020_exports <- ct_search(reporters = "Colombia",
          partners = "All",
...
# Data clean
col_2020_exports_clean <- col_2020_exports %>%
    as_tibble() %>%
...
col_2020_exports_clean %>%
    ...
    labs(x        = "",
         y        = "",
         fill     = "",
         title    = "Colombia goods exports by destination as percent of total exports: year 2020",
...
import_col_comp_2021 %>%
  ...
  mutate(`Imports (US dollars)` = scales::dollar(`Imports (US dollars)`),
         NIT                    = str_remove(string = NIT, pattern = "^0+")) %>%
  kable(format = "latex", booktabs = TRUE, caption = "Top 10 companies according to imports (CIF) in 2021 (December is not included)") %>%
  kable_styling(font_size = 6, latex_options = "striped") %>%
  row_spec(row = 0, bold = TRUE)
```
